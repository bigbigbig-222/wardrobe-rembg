...
const API_CONFIG = {
  ...
  getRemoveBgApiUrl: function() {
    // 优先使用localStorage中的自定义URL
    const customUrl = localStorage.getItem('removeBgApiUrl');
    if (customUrl) {
      return customUrl;
    }
    ...
    if (hostname === 'localhost' || hostname === '127.0.0.1') {
      // 本地开发环境
      return 'http://127.0.0.1:5000/remove_background';
    }
    
    // 生产环境使用从配置文件读取的URL
    if (window.REMBG_API_URL) {
      return window.REMBG_API_URL;
    }
  ...
  // 设置自定义API URL
  setRemoveBgApiUrl: function(url) {
    if (url) {
      localStorage.setItem('removeBgApiUrl', url);
    } else {
      localStorage.removeItem('removeBgApiUrl');
    }
  }
};
